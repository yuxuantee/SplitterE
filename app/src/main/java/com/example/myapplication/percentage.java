...
public class percentage extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        EditText et_ta = findViewById(R.id.editTotalAmount2);
        EditText et_np = findViewById(R.id.editNumofPerson2);
        Button btn_c = findViewById(R.id.btn_c);
        Button btn_cal = findViewById(R.id.btn_cal);
        linearLayout = findViewById(R.id.l3);
                    ...
                    try {
                        ...
                        try {
                            for (int i = 0; i < recordList.size(); i++) {
                                t_percentage += recordList.get(i).getNumber();
                            }
                            if (t_percentage == 100) {
                                for (int i = 0; i < recordList.size(); i++) {
                                    double result = totalAmount * (recordList.get(i).getNumber() / 100);
                                    recordList.get(i).setResult(result);
                                }
                                Intent intent = new Intent(percentage.this, result.class);
                                ...
                                Toast.makeText(percentage.this, "Percentage not equal 100%", Toast.LENGTH_LONG).show();
                            }
                        } catch (NumberFormatException e) {
                        ...
                        if (et2View instanceof EditText) {
                            ...
                            if (!name.isEmpty() && !input.isEmpty()) {
                                try {
                                    ...
                                    recordList.add(new Record(name, inputValue, 2));
                                }catch (NumberFormatException e){
